<template>
    <section>
        <div class="stable-coins-cards">
            <div class="header">قیمت لحظه ای ارز دیجیتال</div>
            <v-row class="cards">
                <v-col cols="12" lg="3" sm="6" v-for="currency in currencies.slice(1, 3)" :key="currency.value">
                    <div class="card">
                        <div class="name-change">
                            <div class="name">{{ currency.text }}</div>
                            <div :class="positive > 0 ? 'change-positive' : 'change-negative'">{{ positive }}%</div>
                            <img :src="currency.icon" alt="" class="icon">
                        </div>
                        <div class="price">{{ currency.buy }} تومان</div>
                        <div class="usd">$ {{ new Intl.NumberFormat().format(currency.usd) }}</div>
                    </div>
                </v-col>
                <v-col cols="12" lg="3" sm="6" v-for="currency in currencies.slice(0, 1)" :key="currency.value">
                    <div class="card">
                        <div class="name-change">
                            <div class="name">{{ currency.text }}</div>
                            <div :class="positive > 0 ? 'change-positive' : 'change-negative'">{{ positive }}%</div>
                ...
                <v-col cols="12" lg="3" sm="6" v-for="currency in currencies.slice(6, 7)" :key="currency.value">
                    <div class="card">
                        <div class="name-change">
                            <div class="name">{{ currency.text }}</div>
